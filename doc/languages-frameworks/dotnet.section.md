...
* `nugetDeps` takes a file path (to a JSON file, or, previously, a nix file), a derivation or a list of derivations. A `deps.json` file can be generated using the script attached to `passthru.fetch-deps`, which is the preferred method. All `nugetDeps` packages are added to `buildInputs`.
::: {.note}
