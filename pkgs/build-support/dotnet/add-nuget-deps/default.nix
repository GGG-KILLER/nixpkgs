...

{
  /**
    ...
    Should be a list of calls to `fetchNupkg`:

    ```nix
    { fetchNupkg }:
    [
      (fetchNupkg { pname = "AsyncIO"; version = "0.1.69"; hash = "sha256-JQKq/U71NQTfPuUqj7z5bALe+d7G1o3GcI8kvVDxy6o="; })
    ]
    ```

    or a JSON file with arguments to `fetchNupkg`:
...
let
  ...
  deps =
    if nugetDeps == null then
      ...
      nugetDeps
    else if lib.hasSuffix ".json" nugetDeps then
      builtins.map fetchNupkg (lib.importJSON nugetDeps)
    else
      assert (lib.isPath nugetDeps);
      lib.warn
        "Nix-based NuGet lockfiles have been deprecated, migrate to JSON lockfiles. Deleting the existing lockfile and re-running fetch-deps will generate a JSON lockfile."
